...
export const ConsultationModal: React.FC<ConsultationModalProps> = ({ isOpen, onClose }) => {
  ...
  return (
    <div className="fixed inset-0 z-50 bg-black/60 backdrop-blur-sm flex items-center justify-center p-4 overflow-y-auto animate-in fade-in duration-300">
      <div className="bg-white rounded-[24px] max-w-xl w-full p-6 sm:p-8 relative shadow-2xl border border-zinc-300 my-8">
        ...
        {submitted ? (
          <div className="text-center py-12 space-y-4">
            ...
            <h3 className="text-2xl font-extrabold text-zinc-950">Briefing Request Received</h3>
            <p className="text-xs sm:text-sm text-zinc-700 max-w-md mx-auto">
              Thank you, <span className="font-bold text-black">{formData.name}</span>. CEO Vivek Gupta’s senior strategist will contact you confidentially on <span className="font-bold text-black">{formData.phone}</span> within 2 hours.
            </p>
            <div className="pt-4 text-xs font-bold text-black">
          ...
          <div className="space-y-6">
            ...
            {/* Modal Header */}
            <div>
              <div className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-orange-50 text-orange-800 border border-orange-200 text-xs font-bold uppercase tracking-wider mb-2">
                <ShieldCheck className="w-4 h-4 text-emerald-600" /> 100% Confidential Briefing
              </div>
            ...
            <form onSubmit={handleSubmit} className="space-y-4 text-left">
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                  ...
                  <input
                    ...
                    className="w-full bg-zinc-50 border border-zinc-300 rounded-xl px-3.5 py-2.5 text-sm text-zinc-950 focus:outline-none focus:border-black transition-colors"
                  />
                </div>
              ...
              <button
                type="submit"
                className="w-full bg-gradient-to-r from-orange-600 via-amber-600 to-orange-600 hover:from-orange-500 hover:to-amber-500 text-white font-black py-3.5 rounded-xl uppercase tracking-wider text-sm transition-all duration-300 shadow-xl shadow-orange-600/25 flex items-center justify-center gap-2 border border-orange-400/30"
              >
