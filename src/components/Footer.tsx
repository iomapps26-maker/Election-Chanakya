...
export const Footer: React.FC<FooterProps> = ({ onRouteChange }) => {
  ...
  return (
    <footer className="bg-black text-white border-t border-zinc-800 pt-16 pb-8 relative overflow-hidden">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-10 pb-12 border-b border-zinc-800/80">
          ...
          <div className="lg:col-span-2 space-y-5">
            <div className="flex items-center gap-3">
              <div className="w-10 h-10 rounded-xl bg-gradient-to-tr from-orange-600 via-amber-500 to-orange-400 text-white flex items-center justify-center font-extrabold shadow-lg shadow-orange-600/30">
                <BarChart3 className="w-6 h-6 stroke-[2.5]" />
              </div>
              <div>
                <span className="font-extrabold text-2xl tracking-tight text-white">ELECTION </span>
                <span className="font-extrabold text-2xl tracking-tight text-gradient-orange">CHANAKYA</span>
              </div>
            </div>
            ...
            <div className="flex items-center gap-3 pt-2">
              <span className="px-3 py-1 rounded-md bg-zinc-900 border border-zinc-800 text-xs font-semibold text-white flex items-center gap-1.5">
                <ShieldCheck className="w-3.5 h-3.5 text-orange-400" /> CEO: {COMPANY_INFO.ceo}
              </span>
              <span className="px-3 py-1 rounded-md bg-zinc-900 border border-zinc-800 text-xs font-semibold text-zinc-300 flex items-center gap-1.5">
                <Globe className="w-3.5 h-3.5 text-orange-400" /> {COMPANY_INFO.domain.replace('https://', '')}
              </span>
            </div>
              ...
              {[
                ...
                return (
                  <a
                    ...
                    className="w-10 h-10 rounded-xl bg-zinc-900 border border-zinc-800 flex items-center justify-center text-zinc-400 hover:text-white hover:bg-orange-600 hover:border-orange-500 transition-all duration-300 transform hover:-translate-y-1 shadow-sm"
                  >
          ...
          {/* Col 2: Quick Links */}
          <div>
            <h4 className="text-sm font-bold uppercase tracking-wider text-white pb-3 mb-4 border-b border-zinc-700 inline-block">
              Quick Links
            </h4>
          ...
          <div className="space-y-4">
            <h4 className="text-sm font-bold uppercase tracking-wider text-white pb-3 mb-2 border-b border-zinc-700 inline-block">
            ...
            <div className="space-y-3 text-sm text-zinc-300">
              ...
              <div className="flex items-center gap-3">
                ...
                <a href={`tel:${COMPANY_INFO.phone}`} className="hover:text-white transition-colors text-xs font-semibold text-zinc-200">
                  {COMPANY_INFO.phone}
                </a>
              </div>
              ...
              <div className="flex items-center gap-3">
                ...
                <a href={`mailto:${COMPANY_INFO.email}`} className="hover:text-white transition-colors text-xs text-zinc-300">
                  {COMPANY_INFO.email}
                </a>
              </div>
            </div>
            ...
            <div className="pt-2">
              ...
              <form onSubmit={handleSubscribe} className="relative">
                <input
                  ...
                  className="w-full bg-zinc-900 border border-zinc-800 rounded-xl px-3.5 py-2.5 text-xs text-white placeholder-zinc-500 focus:outline-none focus:border-white transition-colors pr-10"
                  required
                />
                <button
                  type="submit"
                  aria-label="Subscribe"
                  className="absolute right-1 top-1 bottom-1 px-3 bg-gradient-to-r from-orange-600 to-amber-600 hover:from-orange-500 hover:to-amber-500 text-white rounded-lg flex items-center justify-center transition-colors shadow-md"
                >
        ...
        {/* Legal Links Bar */}
        <div className="py-6 border-b border-zinc-800/60 flex flex-wrap items-center justify-between gap-4 text-xs text-zinc-400">
          <div className="flex flex-wrap items-center gap-4 sm:gap-6">
            ...
            <button onClick={() => handleNavClick('disclaimer')} className="hover:text-white transition-colors">
              Disclaimer
            </button>
            <button onClick={() => handleNavClick('cookie-policy')} className="hover:text-white transition-colors">
              Cookie Policy
            </button>
            <button onClick={() => handleNavClick('refund-policy')} className="hover:text-white transition-colors">
              Refund Policy
            </button>
            <button onClick={() => handleNavClick('sitemap')} className="hover:text-white transition-colors">
              Sitemap
            </button>
        ...
        <div className="pt-6 flex flex-col sm:flex-row items-center justify-between gap-4 text-xs text-zinc-500">
          ...
          <div className="flex items-center gap-4">
            ...
            <button
              onClick={scrollToTop}
              className="w-8 h-8 rounded-lg bg-zinc-900 border border-zinc-800 flex items-center justify-center text-zinc-400 hover:text-black hover:bg-white hover:border-white transition-all shadow-sm"
